...
*Example of a data frame with 3 environmental predictors (columns) and 1000 presences (rows)*
...
-   The **niche_border** argument : This can be either NULL or a vector with length equal to 2 \* number of environmental predictors.
    ...
    -   If NULL: The borders of each dimension in the n-dimensional array (ecological space) will be set to the minimum and maximum values of each predictor in the **data** arugment.
...
# 
___
*Example of an array, constructed with breaks calculated using the* **Sturges** *formula for every predictor, and* **niche_border = NULL**

```{r, fig.height = 4, fig.width = 6, fig.align = 'center'}
## creation of the eco space, filled with species's presences
sn_1 = EcoPA::SpeciesNiche(data = data_species, 
                           hist_type  = rep("Sturges", 3))
# This give a list of length 2 :
# sn_1[[1]] = the array
# sn_1[[2]] = the name of each dimensions (environmental predictors)
# sn_1[[3]] = the presences gived as input, roud to the resolution of the array. We strongly recommend to use those presences and the pseudo-absences generated in the next step in your modelisation procedure, as both of them will have the same resolution.
sn_1[[2]]
```
...
# 
___
*Example of an array, constructed with breaks calculated using the* **Sturges** *formula for every predictor, and* **niche_border = c(-10, 10, -5, 10, -20, 20)**
...
# 
___
*Example of an array, constructed with breaks calculated using the* **FD** *(Freedman-Diaconis) formula for every predictor, and* **niche_border = c(-10, 10, -5, 10, -20, 20)**
...
The function output is a list of nb_pa x ratio_pa_in length. In our example, 3 x 5 = 15 dataframes (5 with 200 pseudo-absences, 5 with 500, and 5 with 1000, each set having different inside ratios).

```{r}
pa = PAGeneration(data = sn_3,
                  nb_pa = c(200, 500, 1000),
                  ratio_pa_InOut = c(0, 0.3, 0.5, 0.8, 1))
length(pa)
names(pa)
head(pa[[1]])
```
